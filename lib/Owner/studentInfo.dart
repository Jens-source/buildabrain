...
class _PreschoolersState extends State<Preschoolers> {
  ...
  Future<void> studentInfo(student, mother, father, paidDate) async {
    int birthday =
        DateTime.now().difference(DateTime.parse(student['birthday'])).inDays;
                ...
                child: ListView(
                  children: <Widget>[
                        ...
                        ? ListView.builder(
                            ...
                            itemBuilder: (BuildContext context, j) {
                              String dateTime = DateFormat('yMMMd')
                                  .format(paidDate[j]["timestamp"].toDate());
                    ...
                    FloatingActionButton.extended(
                      ...
                      onPressed: () {
                        UserManagement.updateStudentClass(
                                student['qrCodeUrl'],
                                classStartTime,
                                classEndTime,
                                classType,
                          ...
                          subtitle: Row(
                            children: <Widget>[
                              all[i]['mother'] == 0
                                  ? Container()
                                  : Text(
                                      "Mother ${all[i]['mother']['firstName'].toString()}"),
                              all[i]['father'] == 0
                                  ? Container()
                                  : Text(
                                      "Father ${all[i]['father']['firstName'].toString()}"),
                            ],
                          ),
                          trailing: IconButton(
                            icon: Icon(Icons.edit),
                            onPressed: () {
                              studentInfo(all[i]['children'], all[i]['mother'],
                                  all[i]['father'], all[i]['paidDates']);
                            },
                          ),
                        )));
              })
