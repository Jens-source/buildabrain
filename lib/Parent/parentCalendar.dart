...
class ParentCalendar extends StatefulWidget {
  ...
  @override
  _ParentCalendarState createState() => _ParentCalendarState(
      this.childrenSnap, this.child, this.tabController, this.tabs);
}

class _ParentCalendarState extends State<ParentCalendar>
    with SingleTickerProviderStateMixin {
  _ParentCalendarState(
      this.childrenSnap, this.child, this.tabController, this.tabs);
      ...
      child: Column(
        children: [
          TabBar(
              ...
              tabs: tabs),
          Expanded(
            child: new TabBarView(controller: tabController, children: child),
          )
        ],
      ),
    );
...
class _ChildState extends State<Child> {
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text(
                  "${8 - childTimestamps.documents.length}/8",
                  style: TextStyle(color: Colors.white),
                ),
              ],
            ),
  ...
  initState() {
    ...
    for (int i = 0; i < childTimestamps.documents.length; i++) {
      _events[DateTime.parse(childTimestamps.documents[i].data['date'])] =
          new List(i + 1);
    ...
    DateTime lastDay = previousTimestamps.length != 0
        ? DateTime.parse(previousTimestamps.last)
        : null;
    int missedDates = 0;
        ...
        if (DateFormat("yyy-MM-dd").format(lastDay) ==
          ...
          if (DateFormat("EEEE").format(lastDay) ==
              ...
              DateFormat("EEEE").format(lastDay) ==
                  childSchedules.documents[1].data['classDay'].toString()) {
            missedDates++;
          }
        }
      }
    ...
    while (count > 0) {
      if (childSchedules.documents.length == 2) {
        if (DateFormat("EEEE").format(now) ==
                childSchedules.documents[0].data['classDay'] ||
            DateFormat("EEEE").format(now) ==
                childSchedules.documents[1].data['classDay']) {
          setState(() {
        ...
        children: [
          ...
          new Container(
            ...
            child: Column(children: [
              ...
              childSchedules.documents.length == 1
                  ? Row(
                      children: [
                        _scheduleCard(
                  ...
                  : childSchedules.documents.length == 2
                      ? Row(
                          children: [
                            _scheduleCard(
                              childrenSnap['classType'],
          ...
          Container(
              ...
              child: TableCalendar(
                ...
                calendarStyle: CalendarStyle(
                  ...
                  weekdayStyle: TextStyle(color: Colors.white70, fontSize: 20),
                  weekendStyle: TextStyle(color: Colors.white70, fontSize: 20),
