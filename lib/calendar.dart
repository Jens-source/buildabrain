...
class _CalendarState extends State<Calendar>
  ...
  void initState() {
    ...
    Firestore.instance.collection('schedule').getDocuments().then((docs) {
      for (int i = 0; i < docs.documents.length; i++) {
        setState(() {
          ...
          for (int j = 0;
              j <=
                  DateTime.parse(docs.documents[i].data['endDate'])
                      .difference(
            ...
            if (DateFormat("EEEE").format(
                ...
                DateFormat("EEEE").format(
                    DateTime.parse(docs.documents[i].data['startDate']))) {
              setState(() {
                markedDates.add(DateTime.parse(schedules[i]['startDate'])
                    .add(Duration(days: j)));
              });
            }
          }
  ...
  onSelect(data) async {
    ...
    for (int i = 0; i < schedules.length; i++) {
      if (DateTime.parse(DateFormat('yyyy-MM-dd').format(data))
              .isAfter(DateTime.parse(schedules[i]['startDate'])) ||
          DateTime.parse(DateFormat('yyyy-MM-dd').format(data))
              .isAtSameMomentAs(DateTime.parse(schedules[i]['startDate']))) {
        if (DateTime.parse(DateFormat('yyyy-MM-dd').format(data))
                .isBefore(DateTime.parse(schedules[i]['endDate'])) ||
            DateTime.parse(DateFormat('yyyy-MM-dd').format(data))
                .isAtSameMomentAs(DateTime.parse(schedules[i]['endDate']))) {
          if (DateFormat('EEEE').format(data) ==
              DateFormat('EEEE')
                  .format(DateTime.parse(schedules[i]['startDate']))) {
            setState(() {
    ...
    for (int i = 0; i < dates.length; i++) {
      await Firestore.instance
          .document("schedule/${schedules[i].documentID}")
          .collection('students')
          .getDocuments()
          .then((docs) {
        setState(() {
                      ...
                      children: <Widget>[
                        ...
                        GestureDetector(
                          ...
                          child: AnimatedContainer(
                              ...
                              child: !selected[i]
                                  ? Card(
                                      color: dates[i].data["class"] ==
                                              'preschoolers'
                                          ? Color.fromRGBO(127, 231, 223, 1)
                                          : dates[i].data["class"] == 'junior'
                                              ? Color.fromRGBO(139, 171, 234, 1)
                                              : dates[i].data["class"] ==
                                                      'advanced'
                                                  ? Color.fromRGBO(
                                                      255, 215, 140, 1)
                                      ...
                                      child: Column(
                                        ...
                                        children: <Widget>[
                                          ListTile(
                                            title: Text(
                                              "${dates[i].data['subject']}",
                                              style: TextStyle(
                                                fontWeight: FontWeight.bold,
                                              ),
                                            ),
                                            subtitle:
                                                Text(dates[i].data['class']),
                                          ),
                                        ],
                                      ),
                                    )
