...
class _StaffInfo extends State<StaffInfo> with SingleTickerProviderStateMixin{
  ...
  onSortColumn(int columnIndex, bool ascending){
    ...
    if(columnIndex == 0)
      {
        if(ascending){
            teachers.sort((a, b) => a.data['displayName'].compareTo(b.data['displayName']));
        } else {
          teachers.sort((a, b) => b.data['displayName'].compareTo(a.data['displayName']));
        }
      }
  }
                    ...
                    children: <Widget>[
                      Container(
                        ...
                        Row(
                          children: <Widget>[
                            ...
                            Text("${teacher.data['firstName']} ${teacher
                                .data['lastName']}", style: TextStyle(
                                fontSize: 16
                            ),)
                      ...
                      Container(
                        ...
                        Row(
                          children: <Widget>[
                            ...
                            Text('Buildabrain, Sai 2', style: TextStyle(
                                fontSize: 16
                            ),)
                        ...
                        Row(
                          children: <Widget>[
                            ...
                            Text('${DateFormat('MMMd').format(
                                DateTime.parse(startDate))} - ${DateFormat(
                                'MMMd').format(DateTime.parse(endDate))}',
                              style: TextStyle(
                                  fontSize: 16
                              ),)
                            ...
                            GestureDetector(
                              ...
                              Container(
                                ...
                                child: Container(
                                  ...
                                  child: Center(
                                    child: Text(endDate, style: TextStyle(
                                        fontSize: 16
                                    ),),
                                  ),
                                ),
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          for (int j = 0; j <= dif; j++) {
                            dayDoc.add(0);
                            daysDif.add(DateTime.parse(startDate).add(
                                Duration(days: j)));
                          }


                          await Firestore.instance.collection(
                              'users/${teacher.documentID}/timestamps')
                            ...
                            for (int i = 0; i < daysDif.length; i++) {
                              for (int j = 0; j <
                                  onValue.documents.length; j++) {
                                if (daysDif[i] == DateTime.parse(
                                ...
                                Column(
                                  children: <Widget>[
                                    Container(
                                        ...
                                        child: Center(
                                            child: dayDocs[i] == 0 ? Text("-") :
                                            Text(dayDocs[i].data['time']
                                                .toString())
                                        )
                                    )
                                  ],
  ...
  Future<void> _teacherInfo(teacher) async {
    ...
    await Firestore.instance.collection(
        'users/${teacher.documentID}/timestamps')
        .where('date', isGreaterThanOrEqualTo: DateFormat('yyyy-MM-dd').format(
        DateTime.now().subtract(Duration(days: num))))
        .getDocuments()
        .then((onValue) {
          ...
          for(int i = num; i >= 0; i--){
            ...
            dates.add(DateFormat('yyy-MM-dd').format(DateTime.now().subtract(Duration(days: i))));
          ...
          for(int j = 0; j < dates.length; j++){
            for(int k = 0; k < onValue.documents.length; k++){
              if(DateTime.parse(dates[j]) == DateTime.parse(onValue.documents[k].data['date'])){
                setState(() {
        ...
        builder: (context) {
          return StatefulBuilder(
              builder: (context, setState) {
                ...
                String weekStart = DateFormat('yyyy-MM-dd').format(
                    DateTime.now().subtract(Duration(days: num)));
                  ...
                  body: ListView(
                          ...
                          children: <Widget>[
                            Container(
                              ...
                              child: Center(
                                child: Container(
                                  ...
                                  width: 120,
                                  child: CircleAvatar(
                                    backgroundImage: NetworkImage(teacher
                                        .data['photoUrl']),
                                  ),
                                ),
                              ),
                            ...
                            Row(
                              ...
                              children: <Widget>[
                                new Text(
                                    "${teacher.data['firstName']} ${teacher
                                        .data['lastName']}", style: TextStyle(
                                  fontSize: 20
                                ),),
                            ...
                            Container(
                              child: Center(
                                child: Text("Hours taught", style: TextStyle(
                                  fontSize: 20,
                                ),),
                              ),
                            ),
                            ...
                            Container(
                              child: Center(
                                child: minutes == 0 ? Text("${hours} hours") :
                            ...
                            Container(
                              child: Center(
                                child: Text(
                                  "${weekStart} to ${today} ", style: TextStyle(
                                  fontSize: 18,
                                ),),
                              ),
                            ),
